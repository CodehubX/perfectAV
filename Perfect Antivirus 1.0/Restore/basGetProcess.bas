  ...
  Dim ID As Long
   ...
   While theloop <> 0
      ...
      'MsgBox ProcessPathByPID(proc.th32ProcessID)
                  ...
                  lstView.ListItems(i).SubItems(3).Caption = FileLen(ProcessPathByPID(proc.th32ProcessID)) \ 1024 & " KB"
                  lstView.ListItems(i).SubItems(4).Caption = GetAttribute(ProcessPathByPID(proc.th32ProcessID))
                  lstView.ListItems(i).SubItems(5).Caption = Format(GetMemory(proc.th32ProcessID) \ 1024, "###,###") & " K"
                  lstView.ListItems(i).SubItems(6).Caption = GetBasePriority(proc.th32ProcessID)
